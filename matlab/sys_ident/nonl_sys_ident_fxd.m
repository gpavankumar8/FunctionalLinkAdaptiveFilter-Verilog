...
% Parameters - LMS
mu_LMS = 0.01;       % Step size (1/64)

% RFFKLMS  parameters
RFF_par.muRFFKLMS = 0.15;
RFF_par.D=512;              
RFF_par.sigma = 8;
...
% HSAFLMS parameters
HSAFLMS_par.mu_w_HSAFLMS = 0.0015;       % Step size
HSAFLMS_par.mu_q_HSAFLMS = 0.0035;       % Step size
...
% RCTFLAF parameters
RCTFLAF_par.mu_w = 1/64;     % Step size = 1/128
RCTFLAF_par.mu_a = 1/32;       % Step size = 1/64
P_t = 3;
...
fpath = '/home/user/pavan/projects/adaptfilt/ASIC/HSAFLMS/simulation/inputs/L128_RIR/';
file_write = 0;
...
% 
% log_lut12 = load('/home/user/pavan/phd/work/matlab/data/LUT/log_mitchell_lut_Q4_12.mat');
% log_lut12 = log_lut12.log_mitchell_lut;
% 
...
% logsin16 = logsin16.log2sin_lut6_12;
% logcos16 = logcos16.log2cos_lut6_12;

% LUT Phi
lut_fraclen = 7;
sin_fpath = sprintf('/home/user/pavan/phd/work/matlab/data/LUT/sin_lut_%dquant_Q4_12.mat', lut_fraclen);
cos_fpath = sprintf('/home/user/pavan/phd/work/matlab/data/LUT/cos_lut_%dquant_Q4_12.mat', lut_fraclen);
...
% %     d = conv(d_nl,rir(1:512)./0.05599);         % 512 tap system
%     d = conv(d_nl,rir(31:31+100)./0.05599);         % 128 tap system
% %     d = conv(d_nl,rir(30:2:30+100)./0.05599);         % 64 tap system
%     
...
%     d = zeros(1,length(x));
    ...
    s_vec_fxd_RCTFLAF = zeros(1,L+del+2);
    g_vec_fxd_RCTFLAF = zeros(L+del,RCTFLAF_par.Q);
...
% %         [e_TFLAF_orig(i), g_vec_orig] = TFLAF_orig_filter(x_vec, d(i), w_TFLAF_orig, g_vec_orig, TFLAF_par);
...
% %         if i > 3
            [e_FxdRCTFLAF(i), s_vec_fxd_RCTFLAF, g_vec_fxd_RCTFLAF] = RCTFLAF_filter_fxd(x(i), d(i), w_FxdRCTFLAF, a_FxdRCTFLAF, s_vec_fxd_RCTFLAF, g_vec_fxd_RCTFLAF, RCTFLAF_par, fraclen, wrdlen, sin16, cos16, lut_fraclen);
% % %         end
...
%             [w_TFLAF_orig] = TFLAF_update(w_TFLAF_orig,e_TFLAF_orig(i-del),g_vec_orig(del*TFLAF_par.Q_t+1:end), TFLAF_par);
...
% % %             w_cmp(i,:) = w_old - a_RCTFLAF;
            [w_FxdRCTFLAF, a_FxdRCTFLAF] = RCTFLAF_update_fxd(w_FxdRCTFLAF, a_FxdRCTFLAF, s_vec_fxd_RCTFLAF(del+1:end), g_vec_fxd_RCTFLAF(del+1:end,:), e_FxdRCTFLAF(i-del), RCTFLAF_par, fraclen, wrdlen);
